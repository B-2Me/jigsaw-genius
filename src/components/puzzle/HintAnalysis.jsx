...
export default function HintAnalysis() {
  const { hints, hintAdjacencyStats, pieces, getSelectionPercentages, stats, mlParams } = useSolver();
    ...
    for (const pieceId in pieceRotationStats) {
      for (const rotation in pieceRotationStats[pieceId]) {
        ...
        if (currentStats.count > 0) {
          const avgScore = currentStats.weighted_avg_contribution;
  ...
  const getAnalysisDescription = () => {
    ...
    return `Data collection mode is active with fair piece distribution.`;
  };
